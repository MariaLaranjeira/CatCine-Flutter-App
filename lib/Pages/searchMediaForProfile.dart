...
class SearchMediaProfile extends StatefulWidget {
  List<Media> profileLists;
  SearchMediaProfile({Key? key, required this.profileLists}) : super(key: key);

  @override
  State<SearchMediaProfile> createState() => _SearchMediaProfileState();
}

class _SearchMediaProfileState extends State<SearchMediaProfile> {
  ...
  getColor(Media media) {
    if (widget.profileLists.contains(media)) {
  ...
  listViewProvider() {
    if (searchedTitle.isNotEmpty) {
        ...
        itemBuilder: (context, index) => ListTile(
          ...
          title: Container(
            ...
            child: RawMaterialButton(
              onPressed: () {
                if (widget.profileLists.contains(displayList[index])) {
                  setState(() {
                    widget.profileLists.remove(displayList[index]);
                  });
                }
                else {
                  setState(() {
                    widget.profileLists.add(displayList[index]);
                  });
                }
              },
              child: Row(
      ...
      return ListView.builder(
        itemCount: widget.profileLists.length,
        itemBuilder: (context, index) => ListTile(
          contentPadding: const EdgeInsets.all(8.0),
          title: Container(
            ...
            child: RawMaterialButton(
              onPressed: () {
                setState(() {
                  widget.profileLists.remove(mediaCat[index]);
                });
              },
              child: Row(
                children: [
                  ClipRRect(
                    ...
                    child: SizedBox(
                      ...
                      child: Image(
                        fit: BoxFit.fill,
                        isAntiAlias: true,
                        image:  getPosterURL(widget.profileLists[index]),
                        semanticLabel: "${widget.profileLists[index].mediaName}...",
                        loadingBuilder: (context, child, progress) {
                          return progress == null ? child : const LinearProgressIndicator();
                        },
                      ),
                  ...
                  Expanded(
                    child: RichText(
                      text : TextSpan (
                          children: <TextSpan> [
                            TextSpan(text:"${getTrimmedName(widget.profileLists[index])}\n",style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 20)),
                            TextSpan(text:widget.profileLists[index].description, style: const TextStyle(color: Colors.white, fontSize: 16)),
                          ]
                      ),
                      maxLines: 5,
                      overflow: TextOverflow.fade,
